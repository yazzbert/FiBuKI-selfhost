...
class Server extends events_1.EventEmitter {
    ...
    /**
     * Handle SOAP request without running an HTTP server (for serverless environments like AWS Lambda)
     * @param {string|Buffer} body - The SOAP XML request body
     * @param {object} options - Optional configuration
     * @param {object} options.headers - HTTP headers (optional, for SOAPAction extraction)
     * @param {string} options.url - Request URL path (default: '/')
     * @returns {Promise<{statusCode: number, body: string}>} Response with status code and XML body
     */
    async handleRequest(body, options) {
        try {
            const xml = typeof body === 'string' ? body : body.toString();
            const opts = options || {};
            const headers = opts.headers || {};
            const url = opts.url || '/';
            // Create a mock request object for internal processing
            const mockReq = {
                url: url,
                method: 'POST',
                headers: headers,
            };
            const responseCallback = (responseBody, status) => {
                console.log(`>>>> callback called responseBody`, responseBody);
                return { statusCode: status, body: responseBody };
            };
            const resp = this._process(xml, mockReq, null, (processedResult, status) => {
                return responseCallback(processedResult, status);
            });
            ...
            if (err.Fault !== undefined) {
                const mockReq = { url: options?.url || '/', method: 'POST', headers: options?.headers || {} };
                this._sendError(err.Fault, (result, statusCode) => {
                    return { statusCode: statusCode || 500, body: result };
                }, false);
            }
            else {
    ...
    _process(input, req, res, cb) {
        ...
        const callback = (result, statusCode) => {
            ...
            return cb(response.result, statusCode);
        };
        const process = () => {
            if (typeof this.log === 'function') {
    ...
    _executeMethod(options, req, res, callback, includeTimestamp) {
        ...
        const handleResult = (error, result) => {
            ...
            return callback(this._envelope(body, headers, includeTimestamp));
        };
        if (!binding.methods[methodName].output) {
            ...
            console.log(`>>>>>>>>>>>>> callback no return`);
        }
        ...
        if (typeof result !== 'undefined') {
            if (isPromiseLike(result)) {
                result.then((value) => {
                    return handleResult(null, value);
                }, (err) => {
