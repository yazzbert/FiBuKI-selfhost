 ...
 */
function checkPatternSafety(pattern, matchCount, totalTransactions, sourceTransactionCount) {
    ...
    if (patternParts.length > 0) {
        const allPartsGeneric = patternParts.every((part) => GENERIC_BANKING_TERMS.some((term) => part === term || term.startsWith(part) || part.startsWith(term)));
        if (allPartsGeneric) {
            return {
                rejected: true,
...
// Prompt Builder
// ============================================================================
function buildPrompt(partnerName, categoryName, positiveTransactions, negativeTransactions, collisionTransactions) {
...
function buildVerificationPrompt(partnerName, categoryName, proposedPatterns, excludePatterns, dryRunResults, totalTransactions) {
...
// ============================================================================
async function dryRunPatternMatch(userId, partnerId, categoryId, proposedPatterns, positiveIds, negativeIds, categoryMap) {
    ...
    for (const pattern of proposedPatterns) {
        ...
        for (const txDoc of allTxSnapshot.docs) {
            ...
            if ((0, pattern_utils_1.matchPatternFlexible)(pattern.pattern.toLowerCase(), txName, txPartner, txReference)) {
...
exports.learnPartnerCategoryPatterns = (0, https_1.onCall)({
    ...
    try {
        ...
        const prompt = buildPrompt(partnerName, categoryName, positiveTransactions, negativeTransactions, collisionTransactions);
        ...
        const matchesFalsePositive = (pattern) => {
            for (const tx of negativeTransactions) {
                const textToMatch = [tx.name, tx.partner, tx.reference]
                    ...
                    .join(" ")
                    .toLowerCase();
                if (textToMatch && (0, pattern_utils_1.globMatch)(pattern, textToMatch)) {
                    return tx;
                }
            }
            return null;
        ...
        if (candidatePatterns.length > 0) {
            ...
            const dryRunResults = await dryRunPatternMatch(userId, partnerId, categoryId, candidatePatterns, positiveIds, negativeIds, categoryMap);
            ...
            for (const cp of candidatePatterns) {
                ...
                const safety = checkPatternSafety(cp.pattern, matches.length, totalTransactions, positiveTransactions.length);
                if (safety.rejected) {
            ...
            if (safePatterns.length === 0) {
                ...
                // LLM verification
                const verifyPrompt = buildVerificationPrompt(partnerName, categoryName, safePatterns, excludePatterns, dryRunResults, totalTransactions);
                ...
                if (verifyText) {
                    try {
                        ...
                        const verifyResult = JSON.parse(verifyJsonText.trim());
