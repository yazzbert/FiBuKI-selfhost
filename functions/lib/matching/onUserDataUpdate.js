...
exports.onUserDataCreated = exports.onUserDataUpdate = void 0;
 ...
 */
async function syncIdentityPartners(userId, beforeData, afterData) {
    const now = firestore_2.Timestamp.now();
    const updates = {};
    // === Sync Personal Entity ===
    const beforePersonal = beforeData.personalEntity;
    const afterPersonal = afterData.personalEntity;
    if (afterPersonal?.name?.trim()) {
        const personalResult = await syncEntityPartner(userId, "personalEntity", afterPersonal, beforePersonal, now);
    ...
    const beforeCompanies = (beforeData.companies || []);
    const afterCompanies = (afterData.companies || []);
    // Build map of before companies by ID
    const beforeCompanyMap = new Map();
    for (const c of beforeCompanies) {
        if (c.id)
            beforeCompanyMap.set(c.id, c);
    }
    ...
    for (let i = 0; i < afterCompanies.length; i++) {
        ...
        const sourceField = `company:${company.id}`;
        if (company.name?.trim()) {
            const result = await syncEntityPartner(userId, sourceField, company, beforeCompany, now);
    ...
    const companiesNeedUpdate = afterCompanies.some((c, i) => updatedCompanies[i]?.partnerId !== c.partnerId);
 ...
 */
async function syncEntityPartner(userId, sourceField, entity, beforeEntity, now) {
 ...
 */
async function createIdentityPartner(userId, sourceField, entity, now) {
    const newPartner = {
        userId,
 ...
 */
async function deleteIdentityPartner(userId, partnerId) {
 ...
 */
function getAllIdentityNames(userData) {
    const names = [];
    ...
    if (userData.companyName)
        names.push(userData.companyName);
 ...
 */
function getAllIdentityVatIds(userData) {
    const vatIds = [];
    // New format
 ...
 */
function getAllIdentityIbans(userData) {
    const ibans = [];
    // New format
 ...
 */
function hasIdentityEntitiesChanged(before, after) {
    // Check personal entity
    const beforePersonal = before.personalEntity;
    const afterPersonal = after.personalEntity;
    if (JSON.stringify(beforePersonal || {}) !== JSON.stringify(afterPersonal || {})) {
        return true;
    }
    // Check companies
    const beforeCompanies = (before.companies || []);
    const afterCompanies = (after.companies || []);
...
function hasMatchingFieldsChanged(before, after) {
    ...
    if (hasIdentityEntitiesChanged(before, after))
        return true;
 ...
 */
exports.onUserDataCreated = (0, firestore_1.onDocumentCreated)({
    document: "users/{userId}/settings/userData",
    region: CONFIG.REGION,
}, async (event) => {
    const userId = event.params.userId;
    ...
    const personalEntity = data.personalEntity;
    ...
    for (const company of companies) {
        if (company.name?.trim() && !company.partnerId) {
            const partnerId = await createIdentityPartner(userId, `company:${company.id}`, company, now);
    ...
    const identityPartnerIds = data.identityPartnerIds || {};
