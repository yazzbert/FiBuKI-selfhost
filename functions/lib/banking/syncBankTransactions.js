...
exports.syncBankTransactionsCallable = void 0;
...
const getBaseUrl = () => process.env.FINAPI_ENVIRONMENT === "live"
    ? FINAPI_LIVE_URL
    : FINAPI_SANDBOX_URL;
// ============================================================================
// Helper Functions
...
async function refreshUserToken(refreshToken, clientId, clientSecret) {
    ...
    if (!response.ok) {
        ...
        if (message.includes("expired") ||
            ...
            response.status === 400) {
            throw new createCallable_1.HttpsError("failed-precondition", "REAUTH_REQUIRED", { code: "REAUTH_REQUIRED", message });
        }
        throw new Error(message);
    }
    return response.json();
}
async function fetchAllTransactions(userToken, accountIds, minDate, maxDate) {
    ...
    while (hasMore) {
        ...
        const response = await fetch(`${getBaseUrl()}/api/v2/transactions?${params}`, {
...
// Callable Function
// ============================================================================
exports.syncBankTransactionsCallable = (0, createCallable_1.createCallable)({
    ...
    if (source?.userId !== userId) {
        throw new createCallable_1.HttpsError("permission-denied", "Not authorized for this source");
    }
    if (source?.type !== "api" || !source?.apiConfig) {
        throw new createCallable_1.HttpsError("invalid-argument", "Source is not an API-connected account");
    }
    const config = source.apiConfig;
    if (config.provider !== "finapi") {
        throw new createCallable_1.HttpsError("invalid-argument", "Only finAPI sources are supported");
    }
    // ========================================================================
    ...
    if (expiresAt && expiresAt < new Date()) {
        throw new createCallable_1.HttpsError("failed-precondition", "Re-authentication required", { code: "REAUTH_REQUIRED", expiresAt: expiresAt.toISOString() });
    }
    ...
    if (isYearChange) {
        console.log(`[syncBankTransactions] Year changed to ${fromYear}, deleting existing transactions...`);
        ...
        console.log(`[syncBankTransactions] Deleted ${existingTxQuery.docs.length} existing transactions`);
    ...
    console.log(`[syncBankTransactions] Fetching transactions from ${dateFrom} to ${dateTo}`);
    const accountId = parseInt(config.accountId, 10);
    const transactions = await fetchAllTransactions(userToken, [accountId], dateFrom, dateTo);
    console.log(`[syncBankTransactions] Fetched ${transactions.length} transactions`);
    // ========================================================================
    ...
    if (iban) {
        ...
        if (orphanedToReassign.length > 0) {
            console.log(`[syncBankTransactions] Found ${orphanedToReassign.length} orphaned transactions with IBAN ${iban}, reassigning to ${sourceId}`);
            const BATCH_SIZE = 500;
            ...
            console.log(`[syncBankTransactions] Reassigned ${orphanedToReassign.length} orphaned transactions`);
        }
    }
    // ========================================================================
    ...
    const txDocs = transactions.map((tx) => ({
        ...
        // Reference
        reference: tx.endToEndReference ||
            tx.mandateReference ||
            tx.primanota ||
            String(tx.id),
    ...
    if (orphanedTxIds.length > 0) {
        console.log(`[syncBankTransactions] Reassigning ${orphanedTxIds.length} orphaned transactions found during dedup`);
        const BATCH_SIZE = 500;
    ...
    const newTransactions = txDocs.filter((t) => !existingHashes.has(t.dedupeHash));
    console.log(`[syncBankTransactions] ${newTransactions.length} new, ${existingHashes.size} duplicates`);
    // ========================================================================
