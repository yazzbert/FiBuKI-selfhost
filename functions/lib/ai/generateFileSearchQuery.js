 ...
 */
exports.generateFileSearchQuery = (0, https_1.onCall)({
    region: "europe-west1",
    memory: "256MiB",
    timeoutSeconds: 30,
    ...
    if (!transactionName && !transactionPartner && !partnerName && !transactionReference) {
        throw new https_1.HttpsError("invalid-argument", "At least one transaction field is required");
    }
    // Build context for the AI - include all available fields
    const contextParts = [];
    if (partnerName)
        contextParts.push(`Matched partner name: ${partnerName}`);
    if (transactionPartner && transactionPartner !== partnerName) {
        contextParts.push(`Counterparty field: ${transactionPartner}`);
    }
    if (transactionName)
        contextParts.push(`Booking text: ${transactionName}`);
    if (transactionReference)
        contextParts.push(`Reference: ${transactionReference}`);
    if (partnerIban)
        contextParts.push(`IBAN: ${partnerIban}`);
    if (amount) {
        contextParts.push(`Amount: ${Math.abs(amount / 100).toFixed(2)} ${currency || "EUR"}`);
    }
    if (date)
...
ONE word:`;
    ...
    const fallbackQuery = extractSimpleQuery(partnerName, transactionPartner, transactionName, transactionReference);
 ...
 */
function extractSimpleQuery(partnerName, transactionPartner, transactionName, transactionReference) {
...
//# sourceMappingURL=generateFileSearchQuery.js.map