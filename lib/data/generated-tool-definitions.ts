...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    "description": "One-time provenance backfill (#184): stamp the files that were corrected by hand BEFORE corrections recorded themselves, so no re-extraction sweep needs the hand-kept list of them again. The list of documents and the fields each correction set is checked into the code, not passed in — three resolve by file id, the rest by file name. Writes only the marker: no extracted value is read, changed or re-derived, so it cannot alter a figure. Defaults to a dry run: pass dryRun=false to write, after checking the rows resolved to the documents you expect. Idempotent — an entry already stamped comes back as already-stamped and a name matching several files comes back as ambiguous rather than being guessed. Corrections made from here on stamp themselves; this is for the ones that predate that.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "dryRun": {
          "type": "boolean",
          "description": "Default true — resolve and report only, nothing written. Pass false to persist."
        }
      }
    }
  },
